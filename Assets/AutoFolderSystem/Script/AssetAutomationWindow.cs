...
public class AssetAutomationWindow : EditorWindow
{
    ...
    public static void ShowWindow()
    {
        ...
        window.minSize = new Vector2(560, 700);
    ...
    private void OnGUI()
    {
        ...
        selectedTabIndex = GUILayout.Toolbar(selectedTabIndex, new[] { "파일 일괄 정리", "특정 파일 이동", "확장자 관리", "파일 일괄 제거", "폴더 관리", "제외 폴더" });
        EditorGUILayout.Space();
        ...
        // Apply pending rename changes after the loop
    ...
    private void DrawFolderOrganizationTab()
    {
        ...
        {
            foreach (string folderPath in settings.TargetFolders.ToList()) // Create a copy to avoid modification issues
            {
                EditorGUILayout.BeginHorizontal();
    ...
    private void DrawExcludedFoldersTab()
    {
        ...
        {
            foreach (string folderPath in settings.ExcludedFolders)
            {
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(folderPath, EditorStyles.miniLabel);
                if (AssetDatabase.IsValidFolder(folderPath))
